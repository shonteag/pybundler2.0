...
def version_read():
    settings_file = open(os.path.join(os.path.dirname(__file__), 'lib', 'pybundler', 'settings.py')).read()
...
setup(
    ...
    entry_points = {
        'console_scripts': [
            'pybundler = pybundler.app:main',
            'pybundler-build = pybundler.app:main_build',
            'pybundler-bundle = pybundler.app:main_bundle',
        ],
    },
